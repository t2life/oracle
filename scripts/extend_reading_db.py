...
やること（すべて冪等。何度実行しても同じ状態になる）:
  ...
  2. 新シートを追加または更新する。
     ...
     - `問いの扱いマスタ`    : 問いの性質ごとの語り口・助言の型（2026-09-12）
     - `問いの型マスタ`      : 問いの型ごとの結論文（2026-09-12）
...
FORBIDDEN_ROWS = [
    ...
    ["危険です", "断定", "警告として読まれる"],
    ["警告", "断定", "警告として読まれる"],
    # 当否の断定（2026-09-12）。先行アプリ engine/advice_module.py L25
    # 「当選を保証する断定表現は用いず、行動のタイミング・方法のみを述べる」の移植。
    ["当選し", "断定", "当否の断定。消費者被害の相談事例と同じ表現になる"],
    ["当選する", "断定", "当否の断定。消費者被害の相談事例と同じ表現になる"],
    ["当たります", "断定", "当否の断定。結果の保証と読める"],
    ["高額当選", "断定", "当否の断定。結果の保証と読める"],
]

# --- 問いの扱いマスタ（2026-09-12 承認）--------------------------------------
# ★オーナー指示: **マスタは判定語と型だけを持つ**。文面の実体は必ずカードから引く
#   （属性・エレメント・キーワード）。カードと無関係な定型文をここへ置くことは禁止。
#   ∴ 差し込み語は文脈パターンDBと同じものだけを使う（`_fill_placeholders`）。
#
# なぜ要るか: 相談内容がどの `質問タイプ分類` にも当たらないと、
# 文脈テンプレが**日付で選ばれて**いた（宝くじの問いに節約の話が出た）。
# 問いを読まずに語るくらいなら、カードそのものを語って直感へ委ねるほうが良い。
#
#   運任せ … 抽選・確率など、当人の行動では動かせない問い。当否は語らない
#   分類外 … どの質問タイプにも当たらなかった問い。カードを語り直感へ委ねる
STANCE_HEADER = ["区分", "キーワード", "語り口", "助言"]
# 区切りは「、」（import_card_db._split_words の仕様）。
# ★「当たる」は「思い当たる」等に誤爆するため採らない（名詞を中心に置く）。
STANCE_KEYWORDS_LUCK = (
    "宝くじ、くじ、抽選、当選、当籤、一等、ギャンブル、賭け、馬券、競馬、競輪、"
    "競艇、パチンコ、スロット"
)
# --- 問いの型マスタ（2026-09-12 承認ウ 第1段）--------------------------------
# ★羅列をやめ、**結論から答える**ための型。
#   従来は「導入→各カード→…」と並べるだけで、問いの中身を一度も使っていなかった。
#   ここで問いの主題語を受け、結論を先に置く。
#   差し込み語は `{主題}`（問いから取る）と `{意味}`（カードから取る）だけ。
FORM_HEADER = ["区分", "キーワード", "結論文"]
FORM_ROWS = [
    ["可否", "できます、できる、可能、大丈夫、うまくいく、叶う、受かる、成功する",
     "{主題}について、いま示されているのは「{意味}」という流れです。"],
    ["時期", "いつ、時期、どのくらい、期限、タイミング、いつごろ、何年、何か月",
     "{主題}の時期については、「{意味}」という流れの中にあります。"],
    ["方法", "どうすれば、どうしたら、どうやって、方法、コツ、進め方",
     "{主題}をどう進めるかについて、鍵になるのは「{意味}」です。"],
    ["選択", "どちら、どっち、選ぶ、選択、すべきか、迷って、悩んで",
     "{主題}の選択について、カードが照らしているのは「{意味}」です。"],
    ["気持ち", "気持ち、本音、どう思、思っている、好き、嫌い、心",
     "{主題}をめぐる心の動きは、「{意味}」として現れています。"],
    ["原因", "なぜ、どうして、理由、原因、せい",
     "{主題}の背景にあるのは、「{意味}」という流れです。"],
    # 区分が決まらない問い（キーワード無し＝既定の受け皿）
    ["既定", "",
     "{主題}について、いま見えているのは「{意味}」という流れです。"],
]

STANCE_ROWS = [
    ["運任せ", STANCE_KEYWORDS_LUCK,
     "{神名}が宿すのは{エレメント}の力。{属性}として、{中立的キーワード}のあなたを照らしています。",
     "あなたの中の「{行動提案ワード}」という感覚に従って進めてみてください。"
     "{肯定的キーワード}へと流れが向かうでしょう。"],
    ["運任せ", "",
     "{エレメント}をまとう{神名}が映しているのは、{基本的意味}という流れです。",
     "「{行動提案ワード}」と心が動いたほうへ進んでみてください。"
     "{肯定的キーワード}が、その先で待っているでしょう。"],
    ["運任せ", "",
     "{属性}である{神名}は、{キーワード}のときを告げています。",
     "あなたの直感が「{行動提案ワード}」と囁くなら、その声に従ってみてください。"
     "{肯定的キーワード}へつながっていくでしょう。"],

    ["分類外", "",
     "{神名}が携えているのは{エレメント}の気配。{基本的意味}が、いまのあなたに重なります。",
     "「{行動提案ワード}」という感覚を手がかりにしてみてください。"
     "{肯定的キーワード}が形になっていくでしょう。"],
    ["分類外", "",
     "{属性}の{神名}が示しているのは、{中立的キーワード}という頃合いです。",
     "あなたの中に浮かんだ「{行動提案ワード}」を、そのまま信じてみてください。"
     "{肯定的キーワード}へ道がひらけるでしょう。"],
    ["分類外", "",
     "{神名}のまとう{エレメント}が、{キーワード}をあなたへ手渡しています。",
     "「{行動提案ワード}」と感じたことを、小さく試してみてください。"
     "{肯定的キーワード}がついてくるでしょう。"],
]
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = [
        ...
        f"問いの型マスタ: "
        f"{_write_sheet(wb, '問いの型マスタ', FORM_HEADER, FORM_ROWS)}"
        f"（{len(FORM_ROWS)}件）",
        f"問いの扱いマスタ: "
        f"{_write_sheet(wb, '問いの扱いマスタ', STANCE_HEADER, STANCE_ROWS)}"
        f"（{len(STANCE_ROWS)}件）",
